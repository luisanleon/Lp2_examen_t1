...
public class DlgEquipoDental extends JDialog implements ActionListener {
	...
	void listar() {
		String jpql="Select ed, de.nombre, ed.costo,ed.fecha_adquisicion,de.nombre_completo,de.correo,esp.titulo,ed.estado"+" from Equipodental ed join ed.Dentista de join ed.especialidad esp order by ed.nro_equipo";
		...
		try {
			for(Object[] objects:resultados) {
				Equipodental equipodental=(Equipodental)objects[0];
				String desDentista = (String) objects[1];
				String desProveedor = (String) objects[2];
				
				imprimir("nro equipo....:"+equipodental.getNro_equipo());
				imprimir("nombre....:"+equipodental.getNombre());
				imprimir("costo....:"+equipodental.getCosto());
				imprimir("fecha aquisicion....:"+equipodental.getFecha_adquisicion());
				imprimir("dentista....:"+equipodental.getDentista());
